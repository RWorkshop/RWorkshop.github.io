...
2.   Simulate 1000 values from the distribution obtained in exercise 1.  and print the first 
...
derived in exercise 4. ? Also state the reason for the same. 
...
3. If obese is defined to be BMI above 30, use ``binom.test()`` to calculate 99% confidence interval for proportion of obese people and comment on the likelihood if more than 20 pc are obese. 
...
v2.   Based on the linear regression model fitted in exercise 3, what is the estimate for 
the percentage of ILI-related physician visits for the week computed in exercise 6.  ? 
v3.    What is the relative error between the estimate (prediction calculated in exercise v2.  ) 
and the actual observed value for the week computed in exercise 6 ? 
