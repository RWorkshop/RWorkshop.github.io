...
#### Exercises
1.  (a) Generate a sample of size $n = 180$ from an Exponential distribution 
with parameter λ = 5. 
(b) Calculate the sample median of the sample in part 1. (a). 
(c) Calculate the median of the Exponential distribution with λ = 5. 
(d) Comment on your results in parts 1. (b) and 1. (c). 
 [8] 
The maximum likelihood estimator for \lambda is given by \lambda  = 1/X where X is the sample 
mean. 
2. (a) Estimate the parameter \lambda  using the maximum likelihood estimator and 
the sample in part 1. (a). 
...
(c) Comment on the estimated values in parts 2.(a) and 2.(b). 
...
#### Exercises
1.  State a suitable distribution for the random variable Y. [1] 
2. Calculate, to four decimal places, the probabilities P[k < Y ≤ k + 20] for all 
...
values k = 220, 240, 260, …, 400 based on the suggested Normal distribution. 
 [4] 
(iv) Plot the values calculated in parts 2. and (iii) on the same graph. [4] 
[Hint: you may find the R command lines(…) useful.] 
(v) Discuss your answers in parts 1. –(iv). [5] 
