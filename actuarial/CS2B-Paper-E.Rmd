...
α1= 0.45, assigning the simulated values to a vector called YAR.  
...
the two time series models, YMA and YAR, generated in parts 1.  and 2. .  
4.  Comment on the general features of the plots in part 3. with reference to 
...
5. Determine the numerical values for the sample ACF at lag 2, for each of the 
two time series models, YMA and YAR, generated in parts 1.  and 2. .  
...
3.  Generate an additional vector, 𝑍, which is of the same length as X, such that 𝑍
...
exponential distribution of some unknown rate $\lambda$. The actuary needs to estimate $\lambda$ 
using only the claim amounts recorded in vector Y. 
4.  Construct R code that calculates the log-likelihood, as a function of the 
parameter $\lambda$, given the claim amounts data in vector Y.  
(v) Determine the value of $\lambda$ at which the log-likelihood function reaches its 
maximum. 
...
2.  Determine the coordinates (x1A, x2A) of the centre of cluster A and the 
coordinates (x1B, x2B) of the centre of cluster B.  
3.  Construct a new column in the data set, portfolio, called ***dist_A*** containing the 
Euclidean distances between the policyholders and the centre of cluster A.  
4.  Construct a new column in the data set, portfolio, called dist_B containing the 
