...
Paste your R Code, Outputs and plots/ graphs in your answer sheet.
Q. 1) Use the data file “Mortality_Investigation.csv” to answer the below questions. This table 
...
central exposed to risk 𝐸37
𝑐
.
5.  Number of lives who did not contribute at all towards the central exposed to risk 𝐸37
𝑐
.
6.  Total number of years of contribution to 𝐸37
𝑐
...
Q. 2) 1. Simulate a time series of length 200 observations in a line chart using the following R 
...
3.   Fit AR(1), AR(2), MA(1) and ARMA(1,1) models to the observations from (2.  and 
comment on the best fitting model using AIC or any other metric.
4.   Using the best fitted model from (3.   create a forecast of the observations for the next 
3 periods.
...
Q. 3) The number of claims in a year on an individual policy follows a Poisson distribution with 
...
retention limit based on the computations of (4.   and the proportion of total claims to 
...
Q. 4) Consider the data set ‘Covid_2019.csv’ (name it covid19) where the first row of the csv file 
...
2.  Create a new data frame “Covid_Cluster” containing only the above mentioned 
columns. Normalize all the columns of the data frame using the scale function. 
3.   Classify the countries into five groups by using the values obtained from (2.  applying 
