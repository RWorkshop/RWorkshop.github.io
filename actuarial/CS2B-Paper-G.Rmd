...
2.  Construct R code that calculates the stationary distribution of Xt for values of q from 0.1 to 0.9 inclusive, at intervals of 0.1, pasting your results into your 
answer script.<p> 
...
4. Plot, on a single graph, four line graphs of the values of F_u against x for the values of x specified in exercise 3 when A_greater_than_u = A_exceed_u, 
...
5. Comment on the graph produced in exercise  (iv). 
...
3. Evaluate the appropriateness of the probability value, calculated in exercise  (ii), 
...
7. Comment on the results produced in exercise  (vi) with reference to the effects of 
the two covariates, drug and gender, on the mortality rate. 
(viii) Update the Cox proportional hazards model in exercise  (vi) to include an 
